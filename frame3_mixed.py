...
#Send table to graphs_tables folder. 
    file=open("figures/Mixed_errors_%d_%d.txt"%(nx,nt),"w")
...
#Check max and min at each timestep for each scheme:
    for i in range (nt+1):
        ...
        min_max[1][i-1]=max(FTBS(phiOld.copy(), c, i))
...
#File name reflects resolution to avoid overwriting.
    ...
    plt.savefig('figures/3scheme_mixed_analysis_%d_%d.pdf'%(nx,nt), 
    bbox_inches = "tight")
...
#File name reflects resolution to avoid overwriting.
    input('press return to save file and continue')
    plt.savefig('figures/min_max_analysis_mixed_%d.pdf'%(nx), 
    bbox_inches = "tight")
    return()
main_mixed(40,40,0.2)
main_mixed(10,40,0.8)
