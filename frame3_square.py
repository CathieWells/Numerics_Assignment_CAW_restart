...
#Send table to figures folder. 
    file=open("figures/Square_errors_%d_%d.txt"%(nx,nt),"w")
...
#File name reflects resolution to avoid overwriting.
    ...
    plt.savefig('figures/3scheme_square_analysis_%d_%d.pdf'%(nx,nt), 
    bbox_inches = "tight")
...
#File name reflects resolution to avoid overwriting.
    input('press return to save file and continue')
    plt.savefig('figures/min_max_square_analysis_%d.pdf'%(nx), 
    bbox_inches = "tight")
    
    return()
main_square(10,40,0.8)
main_square(40,40,0.2)
