...
#Sets up a smooth sine wave of two complete oscillations in the given space.
#Parameters are sent from calling framework.
def sineWave(x,period=2):
    "Function defining a smooth sine wave as a function of position, x"
    "transformed to fit two waves into given space, with vertical range"
    "from 0 to 1."
    phi=0.5*np.sin(4*np.pi*x)+0.5
    return phi